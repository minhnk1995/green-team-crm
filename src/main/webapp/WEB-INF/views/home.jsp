			...
			<div class="col-lg-3 col-md-4 card-group-row__col">
				<div class="card card-group-row__card card-shadow">
					<div class="p-2 d-flex flex-row align-items-center">
						...
						<a href="<%=request.getContextPath() + UrlConst.JOB_CREATE%>"
							class="text-dark"> <strong>Create new job</strong>
						</a>
					</div>
				</div>
			</div>
			...
			<a style="cursor: pointer">
				<div>
					<div class="card" style="padding: 20px; border-radius: 25px">
						<h1
							style="border-style: groove; width: fit-content; padding: 6px;">
							${lstJob.getName()}</h1>
						<c:choose>
							<c:when
								test="${LocalDateTime.now().isAfter(lstJob.getEnd_date())}">
								<h4 class="card-header__title flex mb-0"
									style="margin-top: 10px">
									<span>Manager: ${lstJob.getManager().getName() }</span> <span
										style="margin-left: 10%">Number of Participant:
										${Duration.between(lstJob.getStart_date(), lstJob.getEnd_date()).toDays() }</span>
							...
							<c:otherwise>
								<h4 class="card-header__title flex mb-0"
									...
									<span>Manager: ${lstJob.getManager().getName() }</span> <span
										style="margin-left: 10%">Number of Participant:
										${Duration.between(lstJob.getStart_date(), lstJob.getEnd_date()).toDays() }</span>
									<span style="float: right;">${Duration.between(lstJob.getStart_date(), lstJob.getEnd_date()).toDays() }
										days left</span>
								</h4>
								<div class="progress" style="margin-top: 20px; height: 30px">
									<div class="progress-bar bg-info" role="progressbar"
										style="width: ${Math.round((Duration.between(LocalDateTime.now(), lstJob.getEnd_date()).toDays()*100/Duration.between(lstJob.getStart_date(), lstJob.getEnd_date()).toDays())*100)/100 }%;"
										aria-valuenow="0" aria-valuemin="0" aria-valuemax="100">${Math.round((Duration.between(LocalDateTime.now(), lstJob.getEnd_date()).toDays()*100/Duration.between(lstJob.getStart_date(), lstJob.getEnd_date()).toDays())*100)/100 }%</div>
								</div>
							</c:otherwise>
						</c:choose>
					</div>
